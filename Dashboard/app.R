...
server <- function(input, output) {
  ...
  observe({
    ...
    pal       <- colorNumeric("viridis", c(1,66))
    # radius    <- (radius / max(radius))*30000
    # radius    <- 10000
    radius    <- 2500
    
    observe(print(min(colorData)))
    observe(print(max(colorData)))
    observe(print(colorData))
    
    leafletProxy("map", data = summary_data) %>%
      clearShapes() %>%
      #addCircles(~long, ~lat, radius=radius,stroke=FALSE, fillOpacity=0.4, fillColor=pal(colorData)) %>%
      addCircleMarkers(~long, ~lat,radius = ~count,color = pal(colorData),stroke = FALSE, fillOpacity = 0.5) %>%
